/*
 * N-Body Gravitational Simulation - Parallel Implementation with OpenMP
 * 
 * This program simulates N bodies (particles/stars) interacting via Newtonian gravity
 * using multi-core parallel processing with OpenMP.
 * 
 * Parallelization Strategy:
 * - The outer loop in compute_forces() is parallelized across threads
 * - Each thread computes forces for a subset of bodies (no data races)
 * - The update_bodies() loop is also parallelized
 * - Thread-safe: each thread writes to independent memory locations
 * 
 * Compile: g++ -std=c++17 -O3 -fopenmp nbody_parallel.cpp -o nbody_parallel
 * Run: ./nbody_parallel <N> <steps> <dt> <num_threads> <runs>
 * Example: ./nbody_parallel 1000 100 0.01 4 5
 */
...
// ============================================================================
// FORCE COMPUTATION (PARALLEL O(N²) ALGORITHM WITH OPENMP)
// ============================================================================

/**
 * Compute gravitational forces on all bodies - PARALLEL VERSION.
 * 
 * Parallelization Strategy:
 * - The outer loop (over body i) is parallelized with OpenMP
 * - Each thread processes a subset of bodies independently
 * - NO DATA RACES: Each thread writes only to forces[i] for its assigned i values
 * - The inner loop (over body j) remains sequential within each thread
 * 
 * Work Distribution:
 * - OpenMP automatically divides the N iterations among available threads
 * - Default schedule is typically 'static' for balanced workload
 * 
 * Complexity: Still O(N²), but divided among multiple cores
 * Expected speedup: Nearly linear for large N (communication overhead is minimal)
 * 
 * @param bodies Vector of all bodies (positions and masses) - READ ONLY
 * @param forces Output: force vectors for each body [Fx, Fy, Fz] - WRITE ONLY (no races)
...
void compute_forces(const std::vector<Body>& bodies, 
    ...
    // PARALLEL LOOP: Distribute bodies among threads
    // Each thread computes forces for a subset of bodies (iterations of i)
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < N; ++i) {
        double Fx = 0.0, Fy = 0.0, Fz = 0.0;
        
        // Inner loop: Sum forces from all other bodies j
        // This loop is sequential within each thread
        for (int j = 0; j < N; ++j) {
        ...
        // Thread-safe write: Each thread writes only to its own forces[i]
        forces[i][0] = Fx;
        forces[i][1] = Fy;
        forces[i][2] = Fz;
    }
    // Implicit barrier: All threads synchronize here before function returns
}

// ============================================================================
// TIME INTEGRATION (PARALLEL EXPLICIT EULER METHOD)
// ============================================================================

/**
 * Update body positions and velocities using explicit Euler integration - PARALLEL VERSION.
 * 
 * Parallelization:
 * - Loop over bodies is parallelized with OpenMP
 * - Each thread updates a subset of bodies
 * - NO DATA RACES: Each thread writes to independent body elements
 * 
...
void update_bodies(std::vector<Body>& bodies, 
    ...
    // PARALLEL LOOP: Distribute body updates among threads
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < N; ++i) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " <N> <steps> <dt> <num_threads> [runs]\n";
        std::cerr << "  N           : Number of bodies\n";
        std::cerr << "  steps       : Number of time steps\n";
        std::cerr << "  dt          : Time step size\n";
        std::cerr << "  num_threads : Number of OpenMP threads\n";
        std::cerr << "  runs        : Number of timing runs (default: 5)\n";
        std::cerr << "Example: " << argv[0] << " 1000 100 0.01 4 5\n";
        return 1;
    }
    ...
    std::cout << "========================================\n";
    std::cout << "N-Body Simulation - PARALLEL VERSION (OpenMP)\n";
    std::cout << "========================================\n";
    ...
    std::cout << "Parallelization: Outer loop over bodies\n";
    std::cout << "========================================\n\n";
    ...
    #pragma omp parallel
    {
        #pragma omp master
        {
            std::cout << "OpenMP active with " << omp_get_num_threads() << " threads\n\n";
        }
    }
    ...
    for (int run = 0; run < runs; ++run) {
        ...
        if (run == runs - 1) {
            save_bodies(bodies, "final_positions_parallel.txt");
        }
    }
    ...
    std::cout << "----------------------------------------\n";
    std::cout << "Average parallel time (Tp) with " << num_threads 
              << " threads: " << std::fixed << std::setprecision(6) 
              << avg_time << " s\n";
    std::cout << "========================================\n";
    
    // Save timing data to CSV for analysis
    std::ofstream csv("results_parallel.csv", std::ios::app);
    if (csv.is_open()) {
        // Check if file is empty (write header)
        csv.seekp(0, std::ios::end);
        if (csv.tellp() == 0) {
            csv << "N,steps,dt,threads,Tp\n";
        }
        csv << N << "," << steps << "," << dt << "," << num_threads << "," << avg_time << "\n";
        csv.close();
        std::cout << "\nTiming data appended to results_parallel.csv\n";
    }
